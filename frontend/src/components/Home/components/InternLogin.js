...
const InternLogin = () => {
  ...
  const sendOtp = async () => {
    if (formData.mobileNo.length === 10) {
      await axios.post('http://localhost:5000/intern_login', { mobileNo: formData.mobileNo })
        .then(async (response) => {
  ...
  const verifyOtp = async () => {
    ...
    if (enteredOtp === otp) {
      ...
      setOtpSent(false);
      try {
        const response = await axios.post('http://localhost:5000/intern_login', { mobileNo: formData.mobileNo });
      ...
      <div className="container d-flex flex-column justify-content-center align-items-center" style={{ height: '85vh' }}>
        ...
        <div className="border rounded shadow p-3 d-flex flex-column align-items-center bg-white" style={{ width: '100%', maxWidth: '500px' }}>
          ...
          <div className="d-flex justify-content-center align-items-center w-100">
            <Link to='/intern_reg' className='fw-bold text-center text-decoration-none text-primary p-2' style={{ fontFamily: 'sans-serif' }}>
              New User? Register
            </Link>
          </div>
        </div>
