...
useEffect(() => {
  const fetchCertificateData = async () => {
    try {
      setLoading(true);
      const url = `http://localhost:5000/intern_data/${certificateId}`;
      ...
      const certificateInfo = data[0] || {}; // Assuming data is an array with the first element being the actual data
  ...
  const handleSaveClick = async () => {
    try {
      await fetch(`http://localhost:5000/update-certificate/${certificateId}`, {
        method: 'POST',
