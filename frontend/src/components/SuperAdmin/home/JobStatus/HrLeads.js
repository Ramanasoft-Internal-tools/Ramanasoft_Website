...
const SAHrLeads = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:5000/view-companies'); // Adjust the URL as needed
  ...
  const updateStatus = async (jobId, status) => {
    try {
      ...
      await axios.put(`http://localhost:5000/jobs/status`, { status, ids: Array.isArray(jobId) ? jobId : [jobId] });
  ...
  const handleResumeDownload = async (jobId) => {
    try {
      ...
      const response = await axios({
        url: `http://localhost:5000/download-resume/${jobId.jobId}`, // Update the endpoint to match your backend
        method: 'GET',
