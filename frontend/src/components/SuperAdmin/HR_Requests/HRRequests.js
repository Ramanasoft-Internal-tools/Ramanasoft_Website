...
const HRRequests = () => {
  ...
  useEffect(() => {
    fetchCandidates();
    socket.on('hrRequestsUpdate', (updatedCandidates) => {
      console.log(updatedCandidates);
      setCandidates(updatedCandidates);
    });

    return () => {
      socket.off('hrRequestsUpdate');
    };
  }, []);

  const fetchCandidates = async () => {
    try {
      const response = await axios.get("http://localhost:5000/hr-requests");
  ...
  const handleAccept = async (acceptedCandidates) => {
    try {
      const response = await axios.post("http://localhost:5000/accept-hrs", acceptedCandidates);
  ...
  const handleReject = async (rejectedCandidates) => {
    try {
      await axios.post("http://localhost:5000/reject-hrs", rejectedCandidates)
        .then(response => {
