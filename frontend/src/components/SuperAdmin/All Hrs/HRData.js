...
const DisplayHRs = () => {
  ...
  useEffect(() => {
    const fetchHrs = async () => {
      try {
        const response = await axios.get('http://localhost:5000/hr_data');
  ...
  const handleDelete = async (HRid) => {
    try {
      await axios.delete(`http://localhost:5000/delete_hr/${HRid}`);
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://localhost:5000/hr_data/${editingHr.HRid}`, editingHr);
  ...
  const fetchHrJobs = async (HRid) => {
    try {
      const response = await axios.get(`http://localhost:5000/hr_jobs/${HRid}`);
  ...
  const fetchJobDetails = async (jobId) => {
    try {
      const response = await axios.get(`http://localhost:5000/job_details/${jobId}`);
